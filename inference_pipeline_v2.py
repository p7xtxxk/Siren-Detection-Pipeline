...
def merge_timeline(results, win_sec=WIN_SEC, hop_sec=HOP_SEC, min_consecutive=2):
    """
    - min_consecutive: require N consecutive SIREN windows to confirm detection.
    - Merges consecutive windows into clean intervals.
    """
    intervals = []
    current_label = None
    current_start = None
    ...
    for i, (prob, label) in enumerate(results):
        ...
        # Apply hysteresis: only switch to SIREN after min_consecutive
...
if __name__ == "__main__":
    ...
    # Bias-free random selection: model only sees audio, not source
    test_paths = []
